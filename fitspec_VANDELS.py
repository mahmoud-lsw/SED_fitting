...
oldages = np.array([806400000.0, 1434000000.0, 2500000000.0, 3000000000.0]) #508800000.0, 806400000.0, 1139100030.0, 1434000000.0, 1700000000.0, 2000000000.0, 2500000000.0, 3000000000.0, 4000000000.0########
...
f_old_array = 1.01 - np.logspace(-2, 0, 101)
...
for j in range(9):
    ...
    while oldages[j]*(10**-9) < 14.0 - lbtarr[arg] and arg < 499:
        arg = arg+1
    for l in range(3):
        th_flux_array_new = np.loadtxt("models/spec/newconst/age_" + str(newages[l]) + ".txt")[:,:arg+1]
        th_flux_array_old = np.loadtxt("models/spec/oldburst/age_" + str(oldages[j]) + ".txt")[:,:arg+1] #erg/s/A/cm^2
        for i in range(foldmin, foldmax+1):
            ...
            for k in xrange(61):
                EBV = 0.025*k
                if f_old_V == 1.:
                    th_flux_array = np.expand_dims((th_flux_array_old)*np.expand_dims((10**((-EBV*coef)/2.5)).T, axis=2), axis=0)
                else:
                    th_flux_array = np.expand_dims(((f_old_V/(1.-f_old_V))*th_flux_array_old + th_flux_array_new)*np.expand_dims((10**((-EBV*coef)/2.5)).T, axis=2), axis=0)
