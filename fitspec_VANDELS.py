...
def specbin(spec, binn): #bins up spectrum consisting of a column of wavelength values, a column of fluxes and a column of flux errors by factor binn
...
all_obj_fluxerrs =  np.expand_dims(np.expand_dims(binspec[:,2], axis=0), axis=2)
...
for j in range(9): #8 old ages to iterate over
    ...
    for l in range(3):
        ...
        for i in range(foldmin, foldmax+1):
            ...
            for k in range(61):
                EBV = 0.025*k
                th_flux_array = np.expand_dims((th_flux_array_old + th_flux_array_new).T*np.expand_dims((10**((-EBV*coef)/2.5)).T, axis=2), axis=0) #
