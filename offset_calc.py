...
flux_diffs = np.copy(all_obj_fluxes[:, :, 0])
...
for m in range(len(all_obj_fluxes)):
    ...
    for j in range(127): 
        ...
        for l in range(3):
            ...
            if np.min(chivals) < output[m,7]:
                minchi = np.argmin(chivals)
                output[m,:] = np.array([m+1, all_obj_specz[m], z, ages[j], float(tauvals[l]), EBV[minchi], const[minchi, 0], chivals[minchi]])
                flux_diffs[m, :] = obj_fluxes/(th_flux_array[minchi, :]*const[minchi, 0])
    np.savetxt("photoz_offsetcalc.txt", output, header="obj_no spec_z phot_z age tau EBV norm chi")
    np.savetxt("flux_ratios_offsetcalc.txt", flux_diffs)
