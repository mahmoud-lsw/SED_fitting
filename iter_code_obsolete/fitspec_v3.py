# l_bfgs_b fitting with explicit (wrong) gradient. The process by which EBV and Norm are saved is currently wrong.
...
def chisqfunc(param, th_mags, obj_fluxes, obj_fluxerrs):    #param0 = norm, param1 = EBV 
    ...
    dchisqdpar0 =    np.sum(-2*(th_fluxes/param[0]/obj_fluxerrs)*((obj_fluxes - th_fluxes)/obj_fluxerrs))
    dchisqdpar1 =    np.sum((coef*param[0]/2.5)*th_fluxes*((obj_fluxes - th_fluxes)/obj_fluxerrs))
    return chisq, np.array([dchisqdpar0, dchisqdpar1])
            ...
            for j in range(127):
                ...
                for i in range(1, 501):
                    ...
                    if ages[j]*(10**-9) < 14.00 - WMAP9.lookback_time(z).value:
                        ...
                        optresult = opt.fmin_l_bfgs_b(chisqfunc, [1., 1.], args=(th_mags_scaled, obj_fluxes, obj_fluxerrs), bounds=[(-np.infty, np.infty), (0, np.infty)], pgtol=1e-10)#, approx_grad=True, pgtol=1e-10)
