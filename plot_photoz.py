...
data = np.loadtxt("photoz/photoz_ratios_EBV1.5.txt")
...
good_SFR = (1./(data_nocat[:,4]*10**9))*np.exp(-data_nocat[:,3]/(data_nocat[:,4]*10**9))*data_nocat[:,6]
good_mass = (1. - np.exp(-data_nocat[:,3]/(data_nocat[:,4]*10**9)))*data_nocat[:,6]
...
pylab.scatter(data_nocat[:, 3]*10**-9, data_nocat[:,5], color="blue")
